...
import dev.xiushen.manus4j.ManusApplication;
...
public class ChromeDriverRunner implements ApplicationRunner {
    ...
    public ChromeDriverRunner() {
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            LOGGER.info("JVM shutting down - cleaning up all Chrome processes");
            ...
            try {
                ...
                for (String pid : pids) {
                    try {
                        ...
                        LOGGER.warn("Failed to kill orphaned process with PID: {}", pid, e);
                    }
                }
            } catch (IOException e) {
                LOGGER.warn("Failed to read PID file", e);
            } finally {
                deletePidFile();
            }
        }
    ...
    private void recordProcessId(String pid) {
        try {
            ...
            LOGGER.info("Recorded Chrome process PID: {}", pid);
        } catch (IOException e) {
            LOGGER.error("Failed to record process PID", e);
        }
    }
    ...
    private void killProcessByPid(String pid) {
        try {
            ...
            LOGGER.warn("Failed to kill process with PID: {}", pid, e);
        }
    }

    private String getProcessId(org.openqa.selenium.chrome.ChromeDriver driver) {
        try {
    ...
    private static Boolean checkOS() {
        ...
        if (os.contains("win")) {
            ...
            LOGGER.info("Operating System: Unix/Linux");
            return false;
        } else {
            LOGGER.info("Operating System: Unknown");
            return false;
        }
    }
    ...
    private ChromeDriver createNewDriver() {
        ...
        try {
            ...
            LOGGER.info("Created new ChromeDriver instance with anti-detection");
            return newDriver;
        } catch (Exception e) {
            if (newDriver != null) {
                try {
                    ...
                    LOGGER.warn("Failed to quit failed driver instance", ex);
                }
            }
            LOGGER.error("Failed to create ChromeDriver instance", e);
    ...
    private boolean isDriverActive(ChromeDriver driver) {
        try {
            ...
            LOGGER.warn("Existing ChromeDriver is not active", e);
            closeDriver(driver);
            return false;
        }
    }
    ...
    private void closeDriver(WebDriver driver) {
        try {
            ...
            LOGGER.error("Error closing ChromeDriver", e);
        }
    }
    ...
    private void cleanupAllChromeProcesses() {
        try {
            // 首先尝试正常关闭当前driver
            org.openqa.selenium.chrome.ChromeDriver currentDriver = driver.get();
            ...
            LOGGER.info("Successfully cleaned up all Chrome processes");
        } catch (Exception e) {
            LOGGER.error("Error cleaning up Chrome processes", e);
        }
    }

    @PreDestroy
    public void cleanup() {
        LOGGER.info("Spring container shutting down - cleaning up Chrome resources");
        cleanupAllChromeProcesses();
    }
}